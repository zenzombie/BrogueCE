...
short dialogSelectEntryFromList(
    ...
    short i, selectedButton = -1, len, maxLen;
...
short dialogCreateItemChooseVorpalEnemy(item *theItem){
    char buttonText[COLS];
    box windowSize;
    short i;
    ...
    return dialogSelectEntryFromList(buttons, i, WINDOW_POSITION_MODE_FIXED, WINDOW_POSITION_DUNGEON_TOP_LEFT, WINDOW_SIZE_MODE_FIT_CONTENT, windowSize, "Chose a vorpal enemy:");
}
...
void dialogCreateItemChooseRunic(item *theItem){
    ...
    if (theItem->category & (WEAPON | ARMOR)) {
        ...
        selectedRunic = dialogSelectEntryFromList(buttons, i+1, WINDOW_POSITION_MODE_FIXED, WINDOW_POSITION_DUNGEON_TOP_LEFT, WINDOW_SIZE_MODE_FIT_CONTENT, windowSize, "Choose a runic:");
        ...
        if (selectedRunic >=0 && selectedRunic != noRunic) {
            ...
            if ((theItem->enchant2 == W_SLAYING && theItem->category == WEAPON)
                ...
                selectedVorpalEnemy = dialogCreateItemChooseVorpalEnemy(theItem);
...
short dialogCreateItemChooseKind(enum itemCategory category) {
    ...
    return dialogSelectEntryFromList(buttons, i, WINDOW_POSITION_MODE_FIXED, WINDOW_POSITION_DUNGEON_TOP_LEFT, WINDOW_SIZE_MODE_FIT_CONTENT, windowSize, title);
}
...
void dialogCreateItemChooseEnchantmentLevel(item *theItem) {
    ...
    // If the input was valid, use it. Otherwise use the default.
    if (strlen(inputBuf)                      // we need some input
        ...
        && enchants >= minVal                 // also must be in valid range
        && enchants <= maxVal) {
        if (theItem->category == WAND) {
...
void dialogCreateMonster() {
    ...
    short i, selectedMonster, monsterOffset, buttonCount=0;
    ...
    for (i=0; i < NUMBER_MONSTER_KINDS; i++) {
        monsterKinds[i] = monsterCatalog[i];
        upperCase(monsterKinds[i].monsterName);
    }

    qsort(monsterKinds,NUMBER_MONSTER_KINDS,sizeof(creatureType),&creatureTypeCompareMonsterNames);

    for (i=0; i < NUMBER_MONSTER_KINDS; i+=MONSTERS_PER_PAGE) {
        if (i + MONSTERS_PER_PAGE < NUMBER_MONSTER_KINDS) {
            sprintf(monsterRangeText,"%s - %s",monsterKinds[i].monsterName, monsterKinds[i + MONSTERS_PER_PAGE-1].monsterName);
        } else {
            sprintf(monsterRangeText,"%s - %s",monsterKinds[i].monsterName, monsterKinds[NUMBER_MONSTER_KINDS-1].monsterName);
        }
    ...
    monsterOffset = dialogSelectEntryFromList(buttons, buttonCount, WINDOW_POSITION_MODE_FIXED, WINDOW_POSITION_DUNGEON_TOP_LEFT, WINDOW_SIZE_MODE_FIT_CONTENT, windowSize, "Create monster:");
    if (monsterOffset == -1) {
        return;
    }
    ...
    buttonCount = 0;
    for (i=monsterOffset; i < monsterOffset + MONSTERS_PER_PAGE && i < NUMBER_MONSTER_KINDS; i++) {
    ...
    // choose a monster
    selectedMonster = dialogSelectEntryFromList(buttons, buttonCount, WINDOW_POSITION_MODE_FIXED, WINDOW_POSITION_DUNGEON_TOP_LEFT, WINDOW_SIZE_MODE_FIT_CONTENT, windowSize, "Create monster:");
    ...
    creature *theMonster = generateMonster(monsterKinds[selectedMonster].monsterID, false, false);
    if (theMonster->info.flags & MONST_TURRET) {
    ...
    if (chooseTarget(&selectedPosition, 0, false, true, false, &boltCatalog[BOLT_NONE], &white)) {
        ...
        if (theMonster->info.flags & MONST_TURRET && (!(pmap[selectedPosition.x][selectedPosition.y].layers[DUNGEON] == WALL))) {
            locationIsValid = false;
        }
        if (!(theMonster->info.flags & MONST_TURRET) && cellHasTerrainFlag(selectedPosition.x, selectedPosition.y, T_OBSTRUCTS_PASSABILITY)) {
            locationIsValid = false;
        }
...
void dialogCreateItem() {
    ...
    selectedCategory = dialogSelectEntryFromList(buttons, i, WINDOW_POSITION_MODE_FIXED, WINDOW_POSITION_DUNGEON_TOP_LEFT, WINDOW_SIZE_MODE_FIT_CONTENT, windowSize, "Create item:");
    ...
    sprintf(message,"Your pack now contains %s.", theItemName);
    messageWithColor(message, &itemMessageColor, 0);
}
...
void dialogCreateItemOrMonster() {
    ...
    selectedType = dialogSelectEntryFromList(buttons, 2, WINDOW_POSITION_MODE_FIXED, WINDOW_POSITION_DUNGEON_TOP_LEFT, WINDOW_SIZE_MODE_FIT_CONTENT, (box) {0,0}, "Create:");
